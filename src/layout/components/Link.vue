<template>
    <div class="page tw-bg-cbg-1">
        <a-result :status="status">
            <template #title>
                <div class="tw-text-title-1 tw-pb-4">
                    {{
                        status === "success" ? "外部页面已在新窗口打开" : "打开外部页面被浏览器阻止"
                    }}
                </div>
            </template>
            <template #subtitle>
                <div class="tw-text-text-2">
                    {{
                        status === "success"
                            ? "如未在新窗口打开, 请点击下面 [再次打开] 按钮"
                            : "请检查浏览器设置并点击下面 [再次打开] 按钮重试"
                    }}
                </div>
            </template>
            <template #extra>
                <a-space>
                    ...
                    <a-button
                        :type="status === 'success' ? 'secondary' : 'primary'"
                        @click="onOpenWindow"
                    >
                        <template #icon>
                            <icon-launch />
...
<script setup lang="ts">
    ...
    function onOpenWindow() {
        ...
        //     openedWindow.focus();
        // else
        openedWindow = window.open(route.meta.component_path as string);
        if (openedWindow) {
